    ...
    while(fgets(buffer, sizeof(buffer), fp)) {
      ...
      if(2 == sscanf(buffer, "%31s %31s", key, value)) {
        if(!strcmp(key, "version")) {
          config.version = (unsigned char)atoi(value)&0xff;
          logmsg("version [%d] set", config.version);
        }
        else if(!strcmp(key, "nmethods_min")) {
          config.nmethods_min = (unsigned char)atoi(value)&0xff;
          logmsg("nmethods_min [%d] set", config.nmethods_min);
        }
        else if(!strcmp(key, "nmethods_max")) {
          config.nmethods_max = (unsigned char)atoi(value)&0xff;
          ...
          config.port = (unsigned short)atoi(value)&0xffff;
        ...
        */
          ...
          config.responsemethod = (unsigned char)atoi(value)&0xff;
          logmsg("method [%d] set", config.responsemethod);
        }
        else if(!strcmp(key, "response")) {
          config.connectrep = (unsigned char)atoi(value)&0xff;
...
static int socksconnect(unsigned short connectport,
...
static int tunnel(struct perclient *cp, fd_set *fds)
{
  ...
  if(FD_ISSET(cp->clientfd, fds)) {
    ...
    if(nread > 0) {
      nwrite = send(cp->remotefd, buffer, nread, 0);
      if(nwrite != nread)
        return 1;
  ...
  if(FD_ISSET(cp->remotefd, fds)) {
    ...
    if(nread > 0) {
      nwrite = send(cp->clientfd, buffer, nread, 0);
      if(nwrite != nread)
        return 1;
